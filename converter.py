...
def create_excel_output(invoice_rows: list, receipt_rows: list, month_label: str) -> io.BytesIO:
    ...
    # Fixed column widths (chars) per sheet type
    INV_WIDTHS  = [14, 12, 14, 14, 22, 10, 18, 18, 18]
    REC_WIDTHS  = [14, 12, 12, 22, 10, 18, 18]
    ...
    def write_sheet(ws, headers, rows, col_widths):
        ...
        for r_idx in range(2, ws.max_row + 1):
            for c_idx in range(1, len(headers) + 1):
                ...
                elif c_idx in (2, 3, 4, 6) and len(headers) > 6:
                    # Account numbers (int) – invoices: cols 2,3,4,6
                    cell.number_format = "0"
                    cell.alignment = Alignment(horizontal="center")

                elif c_idx in (2, 3) and len(headers) <= 7:
                    # Receipt account cols
                    cell.number_format = "0"
                    cell.alignment = Alignment(horizontal="center")

                elif c_idx == 5:                        # פרטים (client name)
    ...
    write_sheet(ws_inv, INVOICE_HEADERS, invoice_rows, INV_WIDTHS)

    ws_rec = wb.create_sheet(f"{month_label} קבלות")
    write_sheet(ws_rec, RECEIPT_HEADERS, receipt_rows, REC_WIDTHS)
